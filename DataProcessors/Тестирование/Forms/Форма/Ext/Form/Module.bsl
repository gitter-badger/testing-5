...
Процедура ТестДоступен()
	Если НЕ Объект.Тест.Пустая() И НЕ Тесты.ТестДоступен(Объект.Тест) Тогда
		Сообщить("Данный тест не доступен." + Строка(Объект.Тест));
		Объект.Тест = Справочники.Тесты.ПустаяСсылка();
	КонецЕсли;
КонецПроцедуры
...
Процедура Отвечаю(НомерОтвета)
	Шагомер = 1;
	ВариантыОтветов = Объект.Ответы.НайтиСтроки(Новый Структура("Вопрос", ТекущийВопрос));
	Для Каждого ТекЭлемент Из ВариантыОтветов Цикл
		Если Шагомер = НомерОтвета Тогда
			Для Каждого ТекСтрока Из Объект.Вопросы Цикл
				Если ТекСтрока.Вопрос = ТекущийВопрос Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Учащийся.Пустая() Тогда
		...
		ИначеЕсли Юзеры.ТекущийПользователь(Истина).Пустая() Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Ошибка создания", Отказ);
		Иначе
			Объект.Пользователь	= Юзеры.ТекущийПользователь();
			Объект.Учащийся		= Юзеры.ТекущийПользователь(Истина);
		КонецЕсли;
	КонецЕсли;
	Объект.Дата		= ТекущаяДата();
	Объект.Тест		= Тесты.ТестПоследнийНайти(Объект.Учащийся);
