...
Функция ТекущийПользователь(ФизЛицо=Ложь) Экспорт
	Возврат ?(ФизЛицо, ПараметрыСеанса.ФизЛицо, ПараметрыСеанса.ТекущийПользователь);
КонецФункции
...
Процедура УстановитьПараметрСеансаТекущийПользователь(Знач ИмяПользователя="") Экспорт
	...
	ПараметрыСеанса.ФизЛицо				= ПараметрыСеанса.ТекущийПользователь.ФизЛицо;
...
Функция ПолучитьХеш(Знач Слово)
...
Функция Ключник(Знач Имя, Знач Слово) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Ответ = "Пользователь не найден или пароль введен не верно";
	Если Выборка.Следующий() Тогда
		Если Выборка.ПометкаУдаления Тогда
			Ответ = "Пользователь заблокирован, обратитесь к администратору";
		Иначе
			Юзер = ПользователиИнформационнойБазы.НайтиПоИмени(Имя);
			Если Юзер = Неопределено Тогда
			ИначеЕсли Юзер.СохраняемоеЗначениеПароля = "" И Слово = "" Тогда
				Ответ = "";
			ИначеЕсли Лев(Юзер.СохраняемоеЗначениеПароля, СтрНайти(Юзер.СохраняемоеЗначениеПароля, ",") - 1) = ПолучитьХеш(Слово) Тогда
				Ответ = "";
			ИначеЕсли Сред(Юзер.СохраняемоеЗначениеПароля, СтрНайти(Юзер.СохраняемоеЗначениеПароля, ",") + 1) = ПолучитьХеш(ВРег(Слово)) Тогда
				Ответ = "";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
